...
import { setLoading, setUser } from "../redux/userSlice"; // Import setUser action

const Login = () => {
  const [isLogin, setIsLogin] = useState(false); // Changed to boolean
  ...
  const isLoading = useSelector((store) => store.app.isLoading); // Get isLoading state from store
  ...
  return (
    <div>
      ...
      <form
        ...
        className="flex flex-col w-3/12 p-12 my-36 left-0 right-0 mx-auto items-center justify-center absolute bg-black opacity-90"
      >
